...
These tools are always available, including in [safe mode](/docs/permissions).
...
Available in standard mode. Requires appropriate [permissions](/docs/permissions).
...
Tools from configured [MCP servers](/docs/advanced#mcp-servers) appear as `mcp__<server>__<tool>`.
...
See [Sub-Agents](/docs/advanced#sub-agents) for details.
...
2. nca checks [permissions](/docs/permissions) for the tool
